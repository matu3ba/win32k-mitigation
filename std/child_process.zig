...
pub const ChildProcess = struct {
    ...
    pub const ResourceUsageStatistics = struct {
        ...
        const rusage_init = switch (native_os) {
            .linux, .macos, .ios => @as(?std.posix.rusage, null),
    ...
    fn spawnPosix(self: *ChildProcess) SpawnError!void {
        const pipe_flags = if (io.is_async) posix.O.NONBLOCK else 0;
        ...
        const dev_null_fd = if (any_ignore)
            posix.openZ("/dev/null", posix.O.RDWR, 0) catch |err| switch (err) {
        ...
        const err_pipe = blk: {
            if (native_os == .linux) {
                ...
                break :blk try posix.pipe2(posix.O.CLOEXEC);
            }
        };
